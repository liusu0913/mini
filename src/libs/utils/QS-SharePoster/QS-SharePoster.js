...
function getSharePoster(obj) {
  let {
    ...
    posterCanvasId,
    bgConfig = {
      bgColor: '#fff', width: 375, height: 600, all: 700
    },
    backgroundImage,
    reserve,
    textArray,
...
function drawShareImage(obj) { // 绘制海报方法
  ...
  return new Promise((rs, rj) => {
    try {
      _app.showLoading('填充底色')
      Context.setFillStyle(bgConfig.bgColor || '#fff')
      Context.fillRect(0, 0, bgConfig.width, bgConfig.all)

      _app.showLoading('正在绘制海报')
      _app.log(`背景对象:${JSON.stringify(bgObj)}`)
      if (bgObj && bgObj.path) {
        _app.log('背景有图片路径')
        Context.drawImage(bgObj.path, bgConfig.x || 0, bgConfig.y || 0, bgConfig.width || 375, bgConfig.height || 600)
      } else {
        _app.log('背景没有图片路径')
...
function setFont(textItem = {
  ...
  if (textItem.font && typeof (textItem.font) === 'string') {
    ...
    if (textItem.fontWeight && (typeof (textItem.fontWeight) === 'string' || typeof (textItem.fontWeight) ===
				'number')) { fontWeight = textItem.fontWeight.trim() }
...
function getShreUserPosterBackgroundFc(objs) { // 下载并保存背景图方法
  const {
    backgroundImage,
    type
  } = objs
